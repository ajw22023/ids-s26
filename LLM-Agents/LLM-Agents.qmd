...
- What is RAG? 
    - RAG which stands for Retrieval Augmented Generation, is the solution to the limitations of standard LLMs.This is done by drawing real time data
    from the sources such as APIs. @ferrag2025agents
...
### Finally, Some Code:
This is the first step which wipes any old .JSONL or .CSV files. This is to ensure that if I make any changes to my code, that new files are generated. The new files will then be ready 
to be called when plotting takes place.  The next step, which calls my 'Agents_Multi_step.py' file, runs the directed amount of queries. (36 in this case)
...
# Run agentic + baseline benchmarks
...
## Does API difficulty(Calculator:1, Web search: 2, Weather: 3) predict total_latency?
```{python}
run_regression_comparison(
